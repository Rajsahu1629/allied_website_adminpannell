...
const PublicationPage = () => {
  ...
  const handleMainBannerUpload = async (e) => {
    ...
    try {
      await sendPutRequest("/api/publications/section/update", formData);
  ...
  const handleUpdateBannerAndTitle = async () => {
    ...
    try {
      await sendPutRequest("/api/publications/section/update", {
        title: mainTitle,
  ...
  const handleDeleteProfile = async (id) => {
    ...
    setIsDeleting(true);
    try {
      await sendDeleteRequest(`/api/publications/delete-publication/${id}`);
      await fetchProfiles();
  ...
  const handleSubmit = async (id, formData) => {
    try {
      let data;
      if (id) {
        data = await sendPutRequest(`/api/publications/update-publication/${id}`, formData);
      } else {
        data = await sendPostRequest("/api/publications/create-publication", formData);
      }
      await fetchProfiles();
  ...
  const handleOrderUpdate = async (profileId) => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/publications/update-publication/${profileId}`,
        {
          method: "PUT",
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 w-full overflow-hidden">
      <div className="w-full px-4 sm:px-6 py-6">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-2xl font-semibold text-gray-800">Publications</h1>
          <p className="text-gray-500 mt-1">Manage your publications and resources</p>
        </div>
                    ...
                    {profileData
                      ...
                      .map((profile, index) => (
                        <tr key={profile._id} className="hover:bg-gray-50/50 transition-colors">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 max-w-xs truncate">{profile.subtitle}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{profile.category}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{profile.date ? new Date(profile.date).toLocaleDateString() : ''}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{profile.author}</td>
                          <td className="px-6 py-4 whitespace-nowrap">
