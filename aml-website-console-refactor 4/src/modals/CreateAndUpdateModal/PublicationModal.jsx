...
import { FaFileUpload, FaUpload, FaSpinner, FaFilePdf } from "react-icons/fa";
...
function PdfPreview({ mediaUrl }) {
  const [pdfThumb, setPdfThumb] = React.useState(null);
  ...
  React.useEffect(() => {
    ...
    const loadPdf = async () => {
      try {
        ...
        if (isMounted) setPdfThumb(canvas.toDataURL());
      } catch (e) {
        if (isMounted) setPdfThumb(null);
      }
    };
    loadPdf();
  ...
  if (pdfThumb) {
    return <img src={pdfThumb} alt="PDF preview" className="w-full h-24 object-contain mb-2" />;
  }
  return <FaFilePdf className="text-4xl text-red-500 mb-2" />;
}
...
const PublicationModal = ({
  ...
  useEffect(() => {
    if (profileData) {
      setFormData({
        ...
        type: profileData.type || "publication",
        order: profileData.order || 0,
        subtitle: profileData.subtitle || "",
        category: profileData.category || "",
        date: profileData.date
          ? typeof profileData.date === "string"
            ? profileData.date.slice(0, 10)
            : new Date(profileData.date).toISOString().slice(0, 10)
          : "",
      ...
      if (
        ...
        (profileData.mainImage.frontendUrl || profileData.mainImage.url)
      ) {
        setMainImagePreview(
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Collect IDs of existing media to keep
      const ids = mediaPreviews.map((item) => item._id).filter(Boolean);
      submitData.append("mediaToKeep", ids);
  ...
  const renderMediaPreview = (media, index, isNew = false) => {
    ...
    const mediaUrl = isNew
      ? media.preview
      : `${import.meta.env.VITE_API_BASE_URL}${media.frontendUrl}`;

    // Helper to get PDF name
    const getPdfName = () => {
      if (isNew && media.file && media.file.name) return media.file.name;
    ...
    if (isPdf) {
      return (
        <motion.div
          ...
          className="relative bg-gray-50 rounded-lg overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 group flex flex-col items-center justify-center p-4 min-h-[120px]"
        >
          ...
          <a
            ...
            className="text-xs text-blue-600 underline break-all mb-2 max-w-[120px] text-center"
            title={getPdfName()}
          >
            {getPdfName().length > 20 ? getPdfName().slice(0, 17) + "..." : getPdfName()}
          </a>
          <button
